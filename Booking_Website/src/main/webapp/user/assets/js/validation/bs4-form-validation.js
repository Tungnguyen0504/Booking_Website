    ...
    */
    ...
    lengthCheck(input, minLength, maxLength) {
        if (input.val().length <= minLength) {
            return "Phải lớn hơn " + minLength + " kí tự. ";
        } else if (input.val().length >= maxLength) {
            return "Phải nhỏ hơn " + maxLength + " kí tự. ";
        } else {
            return "";
        }
    }
    ...
    illegalCharCheck(input, illegalCharArray) {
        ...
        if (illegalsUsed === "") {
            ...
            return "Không thể sử dụng:" + illegalsUsed + ". ";
        }
    }
    ...
    specialCharCheck(input) {
        if (!input.val().match(/\W|_/g)) {
            return "Phải bao gồm 1 ký tự đặc biệt. ";
        } else {
            return "";
        }
    }
    ...
    capitalCheck(input) {
        if (input.val().match(/[A-Z]+/)) {
            ...
            return "Phải bao gồm 1 chữ cái in hoa. ";
        }
    }
    ...
    checkAll() {
        $(this.form).submit((e) => {
            ...
            $(this.inputLog).each((i) => {
                ...
                if (invalidString) {
                    this.showWarning(input, inputId, invalidString);
                    this.submitDisabled(true, "Lỗi, vui lòng kiểm tra lại biểu mẫu.");
                    // Stop submission
                    e.preventDefault();
                }
